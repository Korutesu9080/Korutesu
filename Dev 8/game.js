...
/*
FIREWALL — Dual Firewall Version
Adds:
- More shields
- Helper firewall node that shoots with player
*/
...
const COLOR_BG = 0x0B1E2D;
const COLOR_PLAYER = PS.COLOR_CYAN;
const COLOR_HELPER = 0x00E5FF;
...
function draw() {
    ...
    // Helper firewall node
    if (helperActive && playerX < GRID_W - 1) {
        PS.color(playerX + 1, PLAYER_Y, COLOR_HELPER);
    }
}
...
PS.keyDown = function (key) {
    ...
    if (key === PS.KEY_ARROW_LEFT && playerX > 0) {
        playerX--;
    }

    else if (key === PS.KEY_ARROW_RIGHT && playerX < GRID_W - 1) {
        playerX++;
    }
